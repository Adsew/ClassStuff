...
// Override the writeCreate and readCreate methods from the ISerializable interface
// Doing this should write out the starting location for the GameObject.
void Transform::writeCreate(RakNet::BitStream& bs) const
{
	bs.Write(_transform.getPosition());
	bs.Write(_transform.getRotation());
	bs.Write(_transform.getScale());
}
...
void Transform::sendPositionPacket()
{
	...
	//bs.Write((unsigned char)Transform::NetworkPackets::TRANSFORM_POSITION);
...
void Transform::processPacket(RakNet::BitStream& bitStream)
{
	...
	//switch (packetID)
	//{
	//case Transform::NetworkPackets::TRANSFORM_POSITION:
	//	{
	//		float x, y;
	//		bitStream.Read(x);
	//		bitStream.Read(y);
