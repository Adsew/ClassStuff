...
void NetworkServer::loadSettings()
{
	...
	doc.LoadFile("Resources/NetworkSettings.xml");
...
void NetworkServer::sendPacket(RakNet::BitStream& bs, PacketPriority priority)
{
	for (int i = 0; i < clientConnections.size(); i++)
	{
		rakInterface->Send(&bs, priority, RELIABLE_ORDERED, 0, clientConnections[i], false);
	}
}
	...
	while (packet != NULL) 
	{
		...
		case ID_NEW_INCOMING_CONNECTION:
			...
			clientConnections.push_back(packet->guid);
			{
				...
				GameObjectManager::Instance().writeCreateSnapShot(bitStream);
