...
{
	bs.Read(UID);
	LOG("Read Created Packet: " << UID);

	unsigned int count = 0;
	bs.Read(count);
	for (int i = 0; i < count; i++)
	{
		STRCODE componentType = 0;
		bs.Read(componentType);

		Component* component = (Component*)CreateObject(componentType);
		component->setGameObject(this);
...
void GameObject::readUpdate(RakNet::BitStream & bs)
{
	...
	bs.Read(componentCount);

	for (int i = 0; i < componentCount; i++)
	{
		STRCODE componentType;
		bs.Read(componentType);
